...
JobGraph::Job - Abstract class for a job
...
sub is_input_appropriate {
	...
	return $self->is_io_appropriate($self->input, 'JobGraph::Job::Input')
}

sub is_output_appropriate {
	my ($self) = @_;
	
	return $self->is_io_appropriate($self->output, 'JobGraph::Job::Output')
}
