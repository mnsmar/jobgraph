package Test::JobGraph::Job;
use strict;

use Test::Most;
use base qw(Test::JobGraph);

use JobGraph::Job::Input;
use JobGraph::Job::Output;
use JobGraph::Job::Log::File;
...
sub input : Test(5) {
	...
	is_deeply $self->obj->input, [JobGraph::Job::Input->new], '... and should contain the correct types';
	
	can_ok $self->obj, 'set_input';
	$self->obj->set_input([JobGraph::Job::Input->new]);
	is_deeply $self->obj->input, [JobGraph::Job::Input->new], '... and should set the correct types'
}
...
sub output : Test(5) {
	...
	is_deeply $self->obj->output, [JobGraph::Job::Output->new], '... and should contain the correct types';
	
	can_ok $self->obj, 'set_output';
	$self->obj->set_output([JobGraph::Job::Output->new]);
	is_deeply $self->obj->output, [JobGraph::Job::Output->new], '... and should set the correct types'
}
...
sub log : Test(4) {
	...
	can_ok $self->obj, 'log';
	isa_ok $self->obj->log, 'JobGraph::Job::Log::File', '... and returned object';
	
	can_ok $self->obj, 'set_log';
	$self->obj->set_log(JobGraph::Job::Log::File->new({
		FILENAME => 't/sample_data/JobGraph_Job_Log_File.txt'
	}));
	isa_ok $self->obj->log, 'JobGraph::Job::Log::File', '... and should set the correct types'
}
...
sub is_input_appropriate : Test(4) {
	...
	$self->obj->set_input([JobGraph::Job::Output->new]);
...
sub is_output_appropriate : Test(4) {
	...
	$self->obj->set_output([JobGraph::Job::Input->new]);
...
sub is_io_appropriate : Test(4) {
	...
	ok $self->obj->is_io_appropriate([JobGraph::Job::Input->new], 'JobGraph::Job::Input'), '... and the check should succeed';
	ok $self->obj->is_io_appropriate([JobGraph::Job::Output->new], 'JobGraph::Job::Output'), '... and the check should fail again';
