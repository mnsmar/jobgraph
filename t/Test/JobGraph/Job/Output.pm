package Test::JobGraph::Job::Output;
use strict;
...
sub check_init : Test(3) {
	...
	isa_ok $self->obj->source, 'JobGraph::Data', '... and source';
}
...
sub source : Test(6) {
	...
	isa_ok $self->obj->source, 'JobGraph::Data', '... and returned object';
	
	can_ok $self->obj, 'check_and_set_source';
	dies_ok {$self->obj->check_and_set_source('Wrong source')} '... and fails to set a wrong value';
	ok $self->obj->check_and_set_source(JobGraph::Data->new), '... and succeeds setting a legitimate value';
	isa_ok $self->obj->source, 'JobGraph::Data', '... and returned object';
}
...
sub source_is_appropriate : Test(3) {
	...
	ok $self->obj->source_is_appropriate(JobGraph::Data->new), '... and succeeds for a legitimate value';
}
