package Test::JobGraph::Job::Output::File;
use strict;

use Test::Most;
use base qw(Test::JobGraph::Job::Output);
...
sub new_object : Test(setup) {
	...
	$self->{OBJ} = JobGraph::Job::Output::File->new({
		NAME       => 'Any_name',
		SOURCE     => JobGraph::Data::File->new({
...
sub check_init : Test(3) { # Override
	...
	isa_ok $self->obj->source, 'JobGraph::Data::File', '... and source';
}
...
sub source : Test(7) { # Override
	...
	isa_ok $self->obj->source, 'JobGraph::Data::File', '... and returned object';
	...
	dies_ok {$self->obj->check_and_set_source(JobGraph::Data->new)} '... and should fail to set a wrong value again';
	ok $self->obj->check_and_set_source(JobGraph::Data::File->new), '... and should succeed setting a legitimate value';
	isa_ok $self->obj->source, 'JobGraph::Data::File', '... and returned object';
}
...
sub source_is_appropriate : Test(4) { # Override
	...
	dies_ok {$self->obj->source_is_appropriate(JobGraph::Data->new)} '... and should fail for a wrong value again';
	ok $self->obj->source_is_appropriate(JobGraph::Data::File->new), '... and should succeed for a legitimate value';
}
...
sub to_input : Test(2) {
	...
	isa_ok $self->obj->to_input, 'JobGraph::Job::Input::File', '... and returned object';
}
